...
def get_min_time_stamp():
    return date_to_time_stamp(year=2010)


# 数据库保存的时间最大的取值
def get_max_time_stamp():
    return date_to_time_stamp(year=2030)


# 检测时间戳是否在范围内
def check_time_stamp(time_stamp):
    if time_stamp < get_min_time_stamp():
        return f"输入的时间戳需要在2010年({date_to_time_stamp(year=2010)})之后"
    if time_stamp > get_max_time_stamp():
        return f"输入的时间戳需要在2030年({date_to_time_stamp(year=2030)})之前"
