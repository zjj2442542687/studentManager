...
class RegularAddRecord(models.Model):
    ...
    start_time = models.TimeField("每天开始的时间", default=timezone.now)
    end_time = models.TimeField("每天结束的时间", default=timezone.now)
    start_date = models.DateTimeField("开始日期时间", default=timezone.now)
    end_date = models.DateTimeField("结束日期时间", default=timezone.now)
